angular.module('ufcManager')
  .controller('rankingController', function($scope, $http, authService) {
    ...
    // ===== ЗАГРУЗКА ВСЕХ ВЕСОВЫХ КАТЕГОРИЙ =====
    $http.get('/api/weight-classes')
      .then(function(response) {
        $scope.weightClasses = response.data;

        // Добавляем виртуальные категории
        $scope.weightClasses.unshift("All", "Women's P4P", "Men's P4P");

        // Выбираем первую по умолчанию
        $scope.selectedClass = $scope.weightClasses[0];

        // Загружаем рейтинг для выбранной категории
        $scope.loadRankings();
      })
    ...
    // ===== ЗАГРУЗКА РЕЙТИНГА ДЛЯ ВЫБРАННОЙ КАТЕГОРИИ =====
    ...
    // ===== ВЫБОР КАТЕГОРИИ (Angular сам обновит UI, а мы подгрузим данные) =====
    $scope.selectWeightClass = function (wc) {
      $scope.selectedClass = wc;
      $scope.loadRankings();
    };
